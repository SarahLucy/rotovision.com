...
/*  Copyright 2010  Iwebux  (url- http://iwebux.com)
...
function pager($items)
{
	...
	if($items > 0) {
		...
		$p->target("admin.php?page=User Control&usersearch=".$_REQUEST['usersearch']);
...
function user_control() {
	...
	$result = $wpdb->get_col( $wpdb->prepare("SELECT $wpdb->users.ID FROM $wpdb->users WHERE ID != 1 and ID NOT IN (select id from ".$wpdb->prefix."usercontrol)"));
...
add_action('deactivate_'.plugin_basename( __FILE__ ),'dropStatus' );
